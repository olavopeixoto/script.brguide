        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[script.brguide] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[script.brguide] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[script.brguide] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
...
class BRplaySource(Source):
    ...
    def __init__(self, addon):
        ...
        self.brplay = brplay.BRplayTVGuideApi()
    ...
    def getDataFromExternal(self, date, progress_callback=None):
        ...
        for channel in channels:
            visible = channel['isFolder'] != 'true' if 'isFolder' in channel else True
            c = Channel(id=channel['channel_id'], title=channel['name'], logo=channel['logo'], streamUrl=channel['streamUrl'], visible=visible)

            if fetch_from_oiplay:
                programs = self.processPrograms(counter, c, date, total_channels, progress_callback)
            else:
                programs = []
                threads.append(workers.Thread(self.processPrograms, programs, counter, c, date, total_channels, progress_callback))
    ...
    def processPrograms(self, counter, c, date, total_channels, progress_callback):

        for program in self.brplay.programs(c.id, tvdate=date):
            ...
            p = Program(
                ...
                program['end'],
                description,
                None,  # program['images_sixteenbynine']['large'],
                program['images_sixteenbynine']['small'] or c.logo,
            )
            yield p

        if progress_callback:
            if not progress_callback(100.0 / total_channels * counter.next()):
